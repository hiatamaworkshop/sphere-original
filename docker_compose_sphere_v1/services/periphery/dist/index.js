...
import { CleanerFishPool, DEFAULT_CLEANER_FISH_CONFIG, DEFAULT_TRANSITION_THRESHOLDS, } from "./cleaner-fish/cleaner-fish.js";
...
import { resolveDecayPreset, getPresetValues } from "./config/decay-presets.js";
import { SanctificationNeuron, } from "./sanctification/index.js";
...
import { MapReferenceRepository, MapProjectionRepository, MapSpatialFieldRepository, } from "./repository/index.js";
...
const { resolved: decayValues, presetName } = resolveDecayPreset(sphereConfig.renal_core.decay);
...
const decayIntensity = sphereConfig.renal_core.decay.decayIntensity ?? 1.0;
...
let currentMetabolicMode = presetName;
...
console.log(`[Config] Decay preset: "${presetName}" × intensity=${decayIntensity} ` +
...
const sphereMode = sphereConfig.metadata?.mode === "sanctuary" ? "sanctuary" : "core";
...
let sanctificationNeuron;
let renalCore;
let pulseBroadcaster;
...
const globalFieldLayer = new GlobalFieldLayer(sphereConfig.physical_constants?.dimension ?? 384, {
...
if (sphereMode === "core") {
    ...
    renalCore = new RenalCore(projectionDB, referenceDB, spatialFields, renalConfig);
    ...
    setInterval(async () => {
        ...
        const loadFactor = Math.max(renalConfig.minLoadFactor, Math.min(2.0, rawLoadFactor * 100));
        ...
        if (shouldObserve && snapshot) {
            ...
            const { ghostified, fossilized, decomposed } = cleanerFishPool.process(allNodes, getCellId, env, transitionThresholds);
            ...
            if (totalTransitions > 0) {
                console.log(`[CleanerFish] Cycle: ghostified=${ghostified.length} ` +
            ...
            // [Data] All inputs from existing subsystem outputs, no new measurements
            {
                ...
                if (sanctificationNeuron.cycles % 30 === 0 || result.sanctify) {
                    ...
                    console.log(`[Sanctification] epoch=${result.epoch} cycle=${sanctificationNeuron.cycles}` +
                ...
                if (sanctificationNeuron.metabolicAutoMode) {
                    ...
                    if (recommended !== currentMetabolicMode) {
                        ...
                        console.log(`[Sanctification] Metabolic mode: ${prev} → ${recommended} [${modeReason}]`);
                    }
                }
            }
            ...
            if (patrolCounter % PATROL_INTERVAL === 0) {
                ...
                const patrolResult = cleanerFishPool.process(patrolNodes, getCellId, { ...env, dbCapacityRatio: Math.max(0.5, env.dbCapacityRatio) }, // Force higher hunger
                ...
                if (patrolTotal > 0) {
                    console.log(`[CleanerFish] Patrol sweep: ghostified=${patrolResult.ghostified.length} ` +
...
const incarnationPipeline = new IncarnationPipeline(gatekeeper, incarnationParser, tagger, packer, bookkeeper);
const nodeIngestBuffer = new NodeIngestBuffer(bookkeeper, config.incarnationBuffer.batchSize, config.incarnationBuffer.flushIntervalMs);
...
const server = new PeripheryServer(incarnationPipeline, // Unified pipeline for both HTTP API and internal agents
config, entryBuffer, // For GatewayServer (EntryRequest vectorization via buffer)
...
server.setOnAgentCountChange((count) => {
...
async function seedSphere() {
    ...
    const rawData = JSON.parse(readFileSync(seedPath, "utf-8")).slice(0, 20);
    ...
    if (existsSync(relicPath)) {
        const relicData = JSON.parse(readFileSync(relicPath, "utf-8"));
