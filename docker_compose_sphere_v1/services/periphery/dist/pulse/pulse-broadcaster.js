...
export class PulseBroadcaster {
     ...
     */
    broadcast(tickCount, projectionDB, spatialFields) {
        if (!this.config.pulse?.enabled || !this.socket)
            return;
        // N tick ごとに broadcast
        if (tickCount % this.config.pulse.intervalTicks !== 0)
            return;
     ...
     * PulsePacket 生成
     */
    generatePacket(tickCount, projectionDB, spatialFields) {
        // 統計収集
        let totalHeat = 0;
        let ghostCount = 0;
        let amberCount = 0;
        ...
        for (const node of projectionDB.values()) {
            ...
            if (node.kind === "active" || node.kind === "relic")
                activeRelicCount++;
        }
        const nodeCount = projectionDB.size;
        const totalFertility = [...spatialFields.values()].reduce((sum, f) => sum + f.fertility, 0);
        // Signal 計算
        const sig = {
     ...
     */
    send(packet) {
        if (!this.socket || !this.config.pulse)
            return;
...
//# sourceMappingURL=pulse-broadcaster.js.map