 ...
 */
function addNoise(value, noiseFactor = 0.1) {
...
export class SphereCoreAdapter {
     ...
     */
    async scanL1(agentVector, radius = 2.0) {
        const perceptionRadius = this.config.basePerceptionRadius * radius;
        ...
        // [Sampling] sampleRatio reduces traversal cost as agent count increases
        const candidates = await this.projectionRepo.queryNearby(agentVector, dynamicLimit * 2, // More candidates for filtering
        perceptionRadius * 2, sampleRatio);
        const results = [];
        for (const { node, distance } of candidates) {
     ...
     */
    async focus(sessionId, nodeId, agentVector) {
     ...
     */
    async findNearbyGhosts(agentVector, excludeNodeId) {
     ...
     */
    async vectorizeKeyword(keyword) {
