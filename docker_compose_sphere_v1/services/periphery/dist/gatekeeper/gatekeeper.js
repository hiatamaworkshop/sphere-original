 ...
 */
import { validateAgainstSchema, validateConstraints, calculatePayloadBytes, } from "../schema/validator.js";
 ...
 */
function toGatekeeperError(err) {
    return {
        code: err.code,
        message: err.message,
        field: err.field,
...
export class Gatekeeper {
    ...
    validate(capsule) {
        ...
        console.log(`[Gatekeeper] >>> start top=${topTierCount} normal=${normalCount} ghost=${ghostCount} total=${totalNodes}`);
        ...
        const schemaErrors = validateAgainstSchema(capsule, this.capsuleSchema, this.registry);
        ...
        if (valid) {
            console.log(`[Gatekeeper] <<< end PASS bytes=${totalPayloadBytes}`);
        }
        else {
            console.log(`[Gatekeeper] <<< end REJECT violations=${errors.length} codes=${errors.map((e) => e.code).join(",")}`);
        }
        return {
            valid,
            errors,
