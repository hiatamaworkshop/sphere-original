...
async function main() {
    ...
    const individualVecs = [];
    for (const text of texts) {
        const output = await extractor(text, { pooling: "mean", normalize: true });
        individualVecs.push(Array.from(output.data));
    }
    // Get batch vectors
    ...
    for (let i = 0; i < texts.length; i++) {
        const start = i * dim;
        batchVecs.push(Array.from(batchOutput.data.slice(start, start + dim)));
    }
    // Compare
...
//# sourceMappingURL=test-batch-embedding.js.map