...
export class ExploreAgent {
    ...
    ws = null;
    sessionId = "";
    position = []; // 384-dim embedding vector
    remainingTime = 0;
    currentLayer = "tutorial";
    // Agent's request context (received from positioned message)
    query = ""; // Agent's own search intent
    tags = []; // Agent's interest areas
    quest; // External quest (optional)
    requestCounter = 0;
    ...
    discoveries = [];
    sensedNodes = []; // Nodes discovered via sense()
    focusedNodes = []; // Nodes inspected via focus()
    // Event log
    eventLog = [];
    // ============================================================
    // Event Logging
    ...
    fastMode = false;
    delayMs = 0;
    constructor(httpUrl = DEFAULT_HTTP_URL, wsUrl = DEFAULT_WS_URL, name = "ExploreAgent", initialQuery = "explore sphere", initialTags = ["explore", "test"], options) {
    ...
    // Message Handling
    // ============================================================
    handleMessage(msg, onWelcome, _onError // reserved for future error handling
    ) {
        switch (msg.type) {
            case "welcome":
            ...
            case "entryError":
                ...
                break;
            case "senseResult":
                this.logEvent("SENSE_RESULT", `nodes=${msg.nodes.length}`);
                this.resolveRequest(msg.requestId, msg);
                break;
            case "scanResult":
                this.logEvent("SCAN_RESULT", `nodes=${msg.nodes.length}`);
                this.resolveRequest(msg.requestId, msg);
                break;
            case "focusResult":
                this.logEvent("FOCUS_RESULT", `node=${msg.node.id.substring(0, 8)}... kind=${msg.node.kind}`);
                this.resolveRequest(msg.requestId, msg);
                break;
            case "evaluateResult":
                this.logEvent("EVALUATE_RESULT", `success=${msg.success}`);
                this.resolveRequest(msg.requestId, msg);
                break;
            case "moveResult":
                const moveRes = msg;
            ...
            case "returnAck":
                ...
                break;
            case "layerChanged":
                this.logEvent("LAYER_CHANGED", `${this.currentLayer} → ${msg.layer}`);
                this.resolveRequest(msg.requestId, msg);
                break;
            case "error":
    ...
    // ============================================================
    async sense(radius) {
        const result = await this.sendRequest("sense", radius ? { radius } : {});
     ...
     */
    async scan() {
        const result = await this.sendRequest("scan", {});
        return result.nodes || [];
    }
    /**
     ...
     */
    async move(step = 0.3, mode) {
        const result = await this.sendRequest("move", { step, mode });
        return result.result;
    }
    async warp(nodeId) {
        const result = await this.sendRequest("warp", { nodeId });
        return result.result;
    }
    async return() {
    ...
    async enterSanctuary() {
        const result = await this.sendRequest("enterSanctuary", {});
        this.currentLayer = result.layer;
        return result.message;
    }
    async enterCore() {
        const result = await this.sendRequest("enterCore", {});
        this.currentLayer = result.layer;
        return result.message;
    }
    // ============================================================
...
//# sourceMappingURL=explore-agent.js.map