 ...
 */
export class StatisticalEnvForge {
    config;
    constructor(config = {}) {
     ...
     */
    forge(request, projDB) {
        // 1. 配置位置を決定
        const position = this.computePlacement(request, projDB);
        if (!position) {
            console.warn(`[EnvForge] Could not determine placement for ${request.anomalyType}`);
            return null;
        }
        // 2. 環境ノード ID 生成
        ...
        console.log(`[EnvForge] Forged environmental: ${envId.slice(0, 8)} ` +
     ...
     */
    computePlacement(request, projDB) {
        if (projDB.size === 0) {
     ...
     * メトリクス基準でノードを選択
     */
    pickNodeByMetric(projDB, criteria) {
        let target = null;
...
//# sourceMappingURL=env-forge.js.map