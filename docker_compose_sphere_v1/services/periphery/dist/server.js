 ...
 */
function createExternalServiceGuard(config, endpoint) {
    return (req, res, next) => {
        ...
        const service = allowedServices.find((s) => s.id === serviceId && s.secret === secret);
        if (!service) {
        ...
        if (!service.allowedEndpoints.includes(endpoint)) {
            console.warn(`[Server] Service ${serviceId} not allowed for endpoint ${endpoint}`);
            res.status(403).json({
                success: false,
    ...
    setupRoutes() {
        ...
        this.app.post("/sphere/contribute", heavyLimiter, async (req, res) => {
            try {
                ...
                const items = isBatch ? capsules : (capsule ? [capsule] : []);
                if (items.length === 0) {
                    res.status(400).json({
                        success: false,
                ...
                console.log(`[Server] External contribution from "${source}": ` +
                ...
                console.log(`[Server] ✅ Contribution processed: ${totalNodes} nodes incarnated` +
        ...
        this.app.get("/dive/validate/:token", (req, res) => {
            ...
            res.json({
                ...
                ...(result.ticket && {
                    ...
                    remainingSeconds: Math.max(0, Math.floor((result.ticket.issuedAt + result.ticket.ttl * 1000 - Date.now()) / 1000)),
                }),
            });
        });
        // ===== Dive Stats Endpoint =====
        ...
        this.app.post("/quest", mediumLimiter, (req, res) => {
            try {
                ...
                const questId = this.questStore.submit({ query, tags }, submitterId);
         ...
         */
        this.app.post("/sphere/forge/environmental", heavyLimiter, createExternalServiceGuard(this.config, "forge/environmental"), async (req, res) => {
            if (!this.projectionDB) {
                res.status(503).json({ error: "ProjectionDB not available" });
                return;
            }
            try {
                const { anomalyType, severity, signal, suggestedAction } = req.body;
                ...
                console.log(`[Server] Forge environmental request: type=${anomalyType} severity=${severity.toFixed(2)}`);
                // Forge the environmental node
                const result = this.nodeForge.forgeEnvironmental({ anomalyType, severity, signal, suggestedAction }, this.projectionDB);
                ...
                console.log(`[Server] ✅ Environmental forged: ${result.node.id.slice(0, 8)} type=${anomalyType}`);
                res.json({
                    success: true,
                    node: {
                        id: result.node.id,
