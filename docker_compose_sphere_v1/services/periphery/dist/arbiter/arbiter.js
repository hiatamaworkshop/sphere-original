 ...
 */
export function computeAscensionScore(h, w) {
    return h + w;
}
/**
...
export class Arbiter {
     ...
     */
    monitorCandidates(projDB, now, queue) {
        const toRemove = [];
        for (const [nodeId, entry] of this.candidateStore) {
            ...
            const currentScore = computeAscensionScore(node.metrics.h, node.metrics.w);
            ...
            if (elapsed >= this.config.ascensionCooldownMs) {
                ...
                // 冷却期間完了 + スコア維持 → Amber 昇格
                console.log(`[Arbiter] Candidate promoted: ${nodeId.slice(0, 8)} ` +
    ...
    shouldRevive(node) {
        ...
        if (hwProduct >= revivalThreshold && node.metrics.d <= dThreshold) {
            console.log(`[Arbiter] Revival candidate: ${node.id.slice(0, 8)} ` +
     ...
     */
    logChanges(changes) {
        const total = changes.ascended.length +
            changes.eroded.length +
            changes.expired.length;
        if (total === 0)
            return;
        console.log(`[Arbiter] Detected: ` +
    ...
    absorbAndCrystallize(node, projDB) {
        // 機能無効 or 設定なし
        if (!this.config.absorptionEnabled)
            return null;
        ...
        if (!node.vector || node.vector.length === 0)
            return null;
        const activeNodes = []; // 選出対象
        const ghostNodes = []; // 削除のみ
        // 1. 近傍ノード収集
        for (const other of projDB.values()) {
            if (other.id === node.id)
                continue;
            // amber, link, relic は対象外
            if (other.kind !== "active" && other.kind !== "ghost")
                continue;
            // vector がないノードはスキップ
            if (!other.vector || other.vector.length === 0)
                continue;
            // 次元が異なる場合はスキップ
            if (other.vector.length !== node.vector.length)
                continue;
        ...
        console.log(`[Arbiter] crystallization: node=${node.id.slice(0, 8)} ` +
