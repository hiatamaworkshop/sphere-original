...
export class Bookkeeper {
    ...
    async ingest(nodes) {
        ...
        console.log(`[Bookkeeper] ingest_start nodes=${nodes.length} vectorized=${vectorized.length}(dim=${dim}) empty=${emptyVec.length}`);
    ...
    async applyTransitions(queue) {
        ...
        for (const node of shouldRevive) {
            ...
            // h, w, d は維持（評価の蓄積は資産）
            await this.projectionRepo.set(node.id, node);
            console.log(`[Bookkeeper] revival id=${node.id.slice(0, 8)} ` +
        ...
        for (const update of flagUpdates) {
            update.node.metrics.flg = Arbiter.applyFlagUpdate(update.node.metrics.flg, update);
            ...
            if (update.resetMetrics) {
                ...
                console.log(`[Bookkeeper] dropout_reset id=${update.node.id.slice(0, 8)} ` +
        ...
        const total = shouldAscend.length +
            shouldErode.length +
            shouldRevive.length +
            flagUpdates.length;
        if (total > 0) {
            console.log(`[Bookkeeper] transitions ascended=${shouldAscend.length} eroded=${shouldErode.length} revived=${shouldRevive.length} flags=${flagUpdates.length}`);
        }
        // 5. Crystallization: 吸収されたノードを ProjDB + RefDB から削除
        if (crystallizations.size > 0) {
            const allAbsorbedIds = [];
            ...
            console.log(`[Bookkeeper] crystallization_absorbed projdb=${allAbsorbedIds.length} refdb=${allAbsorbedIds.length}`);
        }
     ...
     */
    async recordAscensions(nodes, crystallizations = new Map()) {
        if (nodes.length === 0)
            return;
        for (const node of nodes) {
            const snapshot = {
            ...
            const crystalInfo = crystallization
                ...
                : "";
            console.log(`[Bookkeeper] refdb_ascend id=${node.id.slice(0, 8)} heat=${node.metrics.h.toFixed(3)} weight=${node.metrics.w.toFixed(3)}${crystalInfo}`);
        }
     ...
     */
    async applyGhostification(ghostNodes) {
        if (ghostNodes.length === 0)
            return;
        await this.projectionRepo.batchSet(ghostNodes);
        console.log(`[Bookkeeper] projdb_ghostify count=${ghostNodes.length}`);
    }
    /**
     ...
     */
    async applyFossilization(fossilNodes) {
        if (fossilNodes.length === 0)
            return;
        await this.projectionRepo.batchSet(fossilNodes);
        console.log(`[Bookkeeper] projdb_fossilize count=${fossilNodes.length}`);
    }
    /**
     ...
     */
    async applyDecomposition(decompositions) {
     ...
     * @param evaluations NodeEvaluation array from capsule
     */
    async applyEvaluations(evaluations) {
