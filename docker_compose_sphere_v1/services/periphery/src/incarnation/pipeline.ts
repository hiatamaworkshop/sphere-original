...
import type { Gatekeeper, ValidationError } from "../gatekeeper/gatekeeper.js";
 ...
 */
const REFERENCEABLE_KINDS: ReferenceRecord["kind"][] = ["amber", "relic", "active"];
 ...
 */
export class IncarnationPipeline implements IIncarnationPipeline {
  constructor(
    private gatekeeper: Gatekeeper,
    private incarnationParser: IncarnationParser,
    private tagger: Tagger,
    private packer: Packer,
    private bookkeeper: Bookkeeper,
    private refDB?: IReferenceRepository  // Optional for backward compatibility
  ) {}
   ...
   */
  async ingest(capsule: ExperienceCapsule): Promise<IncarnationResult> {
    const totalSeeds =
  ...
  private async validateReferences(capsule: ExperienceCapsule): Promise<ValidationError[]> {
    ...
    for (let i = 0; i < allSeeds.length; i++) {
      ...
      if (seed.sourceNodeId) {
        const sourceError = await this.validateNodeReference(
          seed.sourceNodeId,
          `seed[${i}].sourceNodeId`,
          true  // Check depth for sourceNodeId
        );
      ...
      if (seed.links) {
        for (let j = 0; j < seed.links.length; j++) {
          const linkError = await this.validateNodeReference(
            seed.links[j],
            `seed[${i}].links[${j}]`,
            false  // No depth check for links (they're references, not derivations)
          );
   ...
   */
  private async validateNodeReference(
    nodeId: string,
    fieldPath: string,
    checkDepth: boolean
   ...
   */
  private async getNodeDepth(nodeId: string, maxIterations = 10): Promise<number> {
    ...
    while (currentId && depth < maxIterations) {
      ...
      if (!sourceId) break;  // Reached root
