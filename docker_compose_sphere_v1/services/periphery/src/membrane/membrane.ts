...
export class Membrane {
   ...
   */
  public validate(msg: ValidatableMessage): MembraneResult {
    const errors: MembraneError[] = [];
  ...
  private validateEntry(
    request: EntryRequest | undefined,
    errors: MembraneError[]
  ): void {
    if (!request) {
      errors.push({
  ...
  private validateEvaluate(
    msg: { nodeId: string; h: number; w: number; d: number },
    errors: MembraneError[]
  ): void {
    if (!msg.nodeId || typeof msg.nodeId !== "string") {
      errors.push({
