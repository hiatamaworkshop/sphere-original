...
export class GlobalFieldLayer {
  ...
  constructor(dimension: number = 384, config: Partial<FieldConfig> = {}) {
   ...
   * 空の磁場を生成
   */
  private createEmptyField(): GlobalAmbientField {
    return {
      updatedAt: Date.now(),
   ...
   * 設定された間隔で磁場を更新
   */
  async tick(projDB: IProjectionRepository): Promise<void> {
    this.tickCounter++;
   ...
   * 磁場を即時更新 (tick 間隔を無視)
   */
  async forceUpdate(projDB: IProjectionRepository): Promise<void> {
    this.tickCounter = 0;
    await this.update(projDB);
  }
   ...
   * 磁場計算の本体
   */
  private async update(projDB: IProjectionRepository): Promise<void> {
    ...
    console.log(
      `[GlobalField] update: nodes=${allNodes.length} samples=${samples.length} (${sampleRatio}%) ` +
   ...
   */
  private randomSample(nodes: SphereNode[], size: number): SphereNode[] {
   ...
   */
  private harvest(nodes: SphereNode[]): {
    centroid: number[];
    avgStrength: number;
