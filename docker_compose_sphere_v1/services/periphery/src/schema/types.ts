...
export type FieldType =
  ...
  | "string[]"    // Array of strings (e.g., tags)
  | "array";      // Array of objects (requires "items" reference)
...
export interface FieldDefinition {
  ...
  // === String constraints ===
  maxLength?: number;       // Maximum string length (chars)
  pattern?: string;         // Regex pattern for validation

  // === Array constraints ===
  items?: string;           // Referenced schema name for object arrays
  minItems?: number;        // Minimum array length
  maxItems?: number;        // Maximum array length

  // === Number/Array constraints ===
  min?: number;             // Minimum value (inclusive, for numbers)
  max?: number;             // Maximum value (for numbers) or array length (alias for maxItems)
}
...
export interface SchemaDefinition {
  ...
  $schema?: string;         // "sphere-schema-v1"
