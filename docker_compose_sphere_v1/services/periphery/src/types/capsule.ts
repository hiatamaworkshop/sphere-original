...
export interface ExperienceCapsule {
  // === Schema Contract ===
  schemaVersion: number;    // MUST equal CAPSULE_SCHEMA_VERSION

  // === New Discoveries (require vectorization) ===
  topTier: NodeSeed[];      // Top N high-value nodes (default: 3)
  normalNodes: NodeSeed[];  // Medium-value nodes
  ghostNodes: NodeSeed[];   // Low-value, volatile nodes

  // === Evaluations on Existing Nodes (no vectorization) ===
  evaluations: NodeEvaluation[];  // Opinions without new content
...
export interface NodeSeed {
  // === L1: Header ===
  tags: string[];           // Keywords (vectorized for spatial position)

  // === L2: Summary ===
  summary: string;          // Brief summary (sensed from afar)

  // === L3: Content ===
  content?: string;         // Main content (read when focused)

  // === L4: References ===
  sourceNodeId?: string;    // Derivation origin (knowledge lineage)
  links?: string[];         // Related nodes (max 5, amber/relic/active only)
  ref_url?: string;         // External URL (max 256 chars, http/https only)

  // === Attributes ===
  flags: number;            // 16-bit NodeFlag (from renal-core)
}
...
export interface NodeEvaluation {
  nodeId: string;           // Reference to existing node (RefDB lookup)
...
export interface ParsedNodeSeed extends NodeSeed {
  vector: number[];         // Vectorized from summary (Parser's responsibility)
}
...
export interface ParsedCapsule {
  ...
  evaluations: NodeEvaluation[];  // Pass-through (no Parser processing)
}
...
export interface TaggedNodeSeed extends ParsedNodeSeed {
  ...
  rank: number;
  classificationFlags: number;  // 16bit flags from tags (Tagger's 16bitTechnique)
}
...
export interface TaggedCapsule {
  ...
  evaluations: NodeEvaluation[];  // Pass-through (no vectorization)
}
 ...
 */
export function validateCapsuleSchema(capsule: ExperienceCapsule): boolean {
 ...
 */
export function getCapsuleSchemaError(
  capsule: ExperienceCapsule
): CapsuleSchemaError | null {
