...
export class ReturnHandler {
  ...
  async processReturn(
    autoCapsule: AutoCapsule,
    proposedCapsule?: ExperienceCapsule
    ...
    let finalCapsule: ExperienceCapsule | null = null;
    const errors: string[] = [];
    ...
    let ingestionResult: ReturnResult["ingestionResult"];
   ...
   * [Principle] まず記録、意味は後
   */
  private generateCapsuleFromAuto(autoCapsule: AutoCapsule): ExperienceCapsule {
    ...
    for (const visit of autoCapsule.visits) {
      ...
      if (visit.focusCount === 0) continue;
