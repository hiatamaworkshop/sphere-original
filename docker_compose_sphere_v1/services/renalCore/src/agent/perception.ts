 ...
 */
export function addNoise(value: number, noiseLevel: number): number {
 ...
 */
export function quantizeHeat(heat: number, noiseLevel: number = 0.15): PerceivedHeat {
 ...
 */
export function quantizeCongestion(ratio: number, noiseLevel: number = 0.1): PerceivedCongestion {
  const perceived = addNoise(ratio, noiseLevel);

  if (perceived < 0.1) return "empty";
  if (perceived < 0.3) return "sparse";
  if (perceived < 0.6) return "moderate";
  if (perceived < 0.9) return "crowded";
  return "full";
}
 ...
 */
export function wobbleDirection(baseDirection: Vector3, wobbleFactor: number = 0.3): Vector3 {
  ...
  const randLen = Math.sqrt(
    randomDir[0] ** 2 + randomDir[1] ** 2 + randomDir[2] ** 2
  );
  if (randLen > 0) {
    randomDir[0] /= randLen;
    randomDir[1] /= randLen;
 ...
 */
export function updateRadarPerception(
  agent: SphereAgent,
  nearbyCells: SpatialFieldV2[],
  currentTick: number,
  config: AgentConfig
): RadarData[] {
 ...
 */
export function generateFocusData(
  node: SphereNode,
  evaluationField: AggregatedEvaluation | undefined,
 ...
 */
export function updateFocusPerception(
  agent: SphereAgent,
  nearbyNodes: SphereNode[],
  evaluationFields: Map<string, AggregatedEvaluation>,
  agentCountByNode: Map<string, number>,
  config: AgentConfig
): FocusData[] {
  return nearbyNodes.map(node => generateFocusData(
    node,
    evaluationFields.get(node.id),
    agentCountByNode.get(node.id) ?? 0,
    config
  ));
}
...
export function aggregateEvaluations(
  evaluations: { quality: number; timestamp: number }[],
  config: AgentConfig
): AggregatedEvaluation {
 ...
 */
export function decayEvaluation(
 ...
 */
export function createFocusBuffer(nodeId: string, config: AgentConfig): FocusBuffer {
  return {
    nodeId,
 ...
 */
export function computeSignalDegradation(
  baseSignal: number,
  observerCount: number,
  alpha: number = 0.15
): number {
  if (observerCount <= 0) return baseSignal;
 ...
 */
export function tryJoinFocusBuffer(
  buffer: FocusBuffer,
  agentId: string,
  currentTick: number
): { joined: boolean; position: number } {
  // Already active?
 ...
 */
export function leaveFocusBuffer(buffer: FocusBuffer, agentId: string): void {
 ...
 */
export function processFocusBufferTimeouts(
  buffer: FocusBuffer,
  currentTick: number
): string[] {
 ...
 */
export function getDegradedFocusData(
  ...
  const baseHeatValue =
    baseFocusData.perceivedHeat === "high" ? 0.8 :
 ...
 */
export function createFocusEcho(
  sourceCell: string,
  nodeId: string,
  kind: string,
  heat: number,
...
export function applyPhaseShift(
  ...
  let interpretation: ReceivedEcho["interpretation"];
  if (salience > 0.7) {
 ...
 */
export function processEchoesForAgent(
  echoes: FocusEcho[],
  agent: SphereAgent,
  config: AgentConfig
): ReceivedEcho[] {
  return echoes
    ...
    .filter((echo): echo is FocusEcho => echo !== null)
 ...
 */
export function createAgentPerception(config: AgentConfig): AgentPerception {
  return {
    radar: {
