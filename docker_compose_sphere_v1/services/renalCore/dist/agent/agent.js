...
import { randomUnitVector, addNoise, } from "./perception.js";
// ============================================================
// Agent Factory
// ============================================================
/**
 ...
 */
export function createAgent(name, startCellId, startVector, personalityType = "follower", config) {
 ...
 */
export function createPersonality(type) {
    ...
    const vary = (v) => Math.max(0, Math.min(1, v + (Math.random() - 0.5) * 0.2));
    return {
        type,
 ...
 */
export function getEffectiveType(agent) {
 ...
 */
export function decideAction(agent, radarData, focusData, config) {
    // Check energy first
 ...
 */
function decideFollowerAction(agent, radarData, focusData, config) {
    ...
    if (hotNodes.length > 0) {
        ...
        const accessible = hotNodes.filter(n => n.nearbyAgentCount < config.defaultSoftCapacity * agent.personality.patience);
 ...
 */
function decidePioneerAction(agent, radarData, focusData, config) {
 ...
 */
function decideCriticAction(agent, radarData, focusData, config) {
    ...
    const controversial = focusData
        ...
        .sort((a, b) => (b.aggregatedEvaluation?.uncertainty ?? 0) - (a.aggregatedEvaluation?.uncertainty ?? 0));
 ...
 */
function decideTrustBasedAction(agent, radarData, focusData, config) {
 ...
 */
function decideRandomAction(agent) {
    return {
        type: "move",
        direction: randomUnitVector(),
    };
 ...
 */
export function updateAgentState(agent, action, success, config) {
    // Energy consumption
    if (action.type !== "rest") {
 ...
 */
export function updateInternalState(agent, newInfo) {
 ...
 * Record node visit
 */
export function recordVisit(agent, nodeId, maxVisitedSize = 50) {
 ...
 */
export function createEvaluation(agent, nodeId, focusData, dwellTime) {
 ...
 */
export function createDiscovery(agent, nodeId, focusData) {
 ...
 */
export function createExplorationReport(agent, discoveries, totalTicks) {
    return {
        agentId: agent.id,
 ...
 */
export function createSubmissionCapsule(discoveries, evaluations) {
    ...
    const toNodeSeed = (discovery) => {
        ...
        if (discovery.significance > 0.7)
            tags.push("important");
        if (evaluation?.novelty && evaluation.novelty > 0.5)
            tags.push("novel");
        if (evaluation?.quality && evaluation.quality > 0.5)
            tags.push("quality");
 ...
 */
export function createGhostPulse(agent, intent) {
    return {
        agentId: agent.id,
 ...
 */
export function evaluateDestination(agent, targetData, aggregatedEval, config) {
    ...
    const congestionPenalty = congestion === "full" ? 0.3 :
