...
## SphereContext の配線状況 (2026-02-20 調査)

### 接続済み

- `_layer: ExperienceLayer = "tutorial"` — 初期値 tutorial (L159)
- `evaluate()` — layer 対応済み (L604-617)。`handleLayerEvaluation()` で層別処理
- `enterSanctuary()` / `enterCore()` — 遷移メソッド実装済み (L1268, L1288)
- `return()` — バッファの evaluations を capsule にマージ (L1211)

### 未接続 (フィルタなし)

| メソッド | 現状 | 必要な変更 |
|---------|------|-----------|
| `sense()` (L280) | coreAdapter.sense() の結果をそのまま返す | filterByLayer() を噛ませる |
| `scan()` (L371) | coreAdapter.sense() → そのまま | 同上 |
| `scanL1()` (L397) | coreAdapter.scanL1() → そのまま | 同上 |
| `focus()` (L434) | _sensedNodeIds チェックのみ | layer で kind ガードを追加 |

変更量は極めて小さい。各メソッドに1行のフィルタ挿入で完了する。
...
| コンポーネント | 状態 |
|---|---|
| `ExperienceLayer` 型 | 定義済み (experience-layer.ts) |
| `LAYER_CHARACTERISTICS` | 定義済み (experience-layer.ts) |
| `LayerTransitionManager` | 実装済み (layer-transition.ts) |
| `GatewaySession.layer` | フィールド存在 |
| `SphereContext` フィルタ接続 | **未実装** ← sense/scan/scanL1 に1行追加で完了 |
| eval の層別処理 | **実装済み** (handleLayerEvaluation) |
| 遷移メソッド | **実装済み** (enterSanctuary/enterCore) |
