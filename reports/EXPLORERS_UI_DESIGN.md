...
### 2026-02-16: BroadcastRenderer 実装 + SNS 最適化 (futurePreparation)

Narrative (LLM 生成) と並行して **Broadcast (決定的射影)** を出力する機能を追加。
...
| ファイル | 変更 |
|---------|------|
| `phi-agent/src/broadcast-renderer.ts` | 新規 — renderBroadcast() 純粋関数 + flagsToLabels() |
| `phi-agent/src/agent.ts` | encounters に flags 追加, broadcast 出力 (Step 5b), セパレータ `---` |
| `phi-agent/src/eval-log.ts` | NarrativeEntry に flags?, broadcast? 追加 |
| `explorers/parser.py` | extract_broadcast() 追加, `---` セパレータ対応 |
| `explorers/app.py` | Broadcast パネル追加 |

**Docker Compose 更新:**
- phi-agent, explorers, periphery イメージをリビルド (全 tsc パス)
- docker-compose.yml 自体の変更は不要 (コード変更のみ)

---

### 2026-02-16: UI 簡素化 (hf-deploy)

`explorers/app.py` に以下の調整を適用:

1. **"Powered by Groq API..." 行を削除** — モデル名は固定表示ではなく Dropdown で選択式。特定 API に言及する必要なし
2. **Model info 行を削除** — `info="Groq API model (JSON format supported)"` → 不要な説明を除去
3. **Advanced Settings アコーディオン削除** — `SPHERE_URL` は環境変数で設定すれば十分。UI に露出する必要なし。`launch_agent` のデフォルト引数に移行
4. **"Execution takes 2-4 minutes" → "Execution may take some time"** — 実行時間は環境依存で固定値は不正確
