...
## 6. 防衛設計の原則 (2026-02-20)

### イタチゴッコに完勝する必要はない

防衛策の歴史は常にイタチゴッコ。スフィアには代謝システムがある以上、
過敏になる必要はない。異物は時間経過で自然に排除される (decay + TTL)。
amber もいずれ降格する可能性がある。

ただし **明らかに手落ちな設計は避ける**。自律した生物としてあり得ないからだ。

### 免疫系のアナロジー

| 免疫の層 | スフィアの対応 | 状態 |
|---------|--------------|------|
| 自然免疫 (即時検知) | organicRatio, churnRate, amberSlope | 実装済み |
| 炎症反応 (即時ブロック) | Meta neuron veto → 聖域化阻止 | 実装済み |
| 免疫記憶 (再感染への備え) | **不応期 (Refractory Period)** | 設計済み・未実装 |

現在の弱点: suspicion が `× 0.982^300 ≈ 0.004` で5分後にほぼゼロになる。
「検知 → 5分待機 → 再試行」が最も単純な攻撃パターンだが、これを防げない。

### 多層防御 (Meta 以外の壁)

Meta neuron だけが防壁ではない。以下が独立に機能している:

- **cooldown (5min)**: candidate 登録後の score 維持義務。decay で h が落ちれば dropout
- **lowerThresholdRatio (0.85)**: cooldown 中に閾値の 85% を維持する必要
- **dropout reset**: cooldown 失敗で h=400, w=500 にリセット。再挑戦コストが高い
- **flexibilityHealth**: amber 増加 → Soft neuron の vitality 低下 → 聖域化が遠のく
- **allostatic threshold**: ノード数に応じて閾値が適応的に上昇

完全な穴ではなく、薄い壁がある状態。
不応期の実装はこの壁を適切な厚さにする作業。

### 設計方針

- 過敏にならない — 代謝が最終防衛線
- 明らかな手落ちは避ける — 5分で免疫記憶を失うのは不自然
- 不応期のパラメータは実績から決める — 聖域化サイクルの観測が先
- 恒久ブロックは絶対に作らない — 有限時間で必ず復帰可能
